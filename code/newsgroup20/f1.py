...
class Selector(object):
	...
	def __init__(self, dataset, N_CLUSTERS, THRESHOLD, USE_KMEANS, use_cache=False, cache=None, cache1=None):
		"""
		use_cache: bool for using cache 
		cache: old values to used in...
		"""

		df_train = pd.read_csv("20ng-train-stemmed.txt",names=["c","d"], sep="\t")
		df_test = pd.read_csv("20ng-test-stemmed.txt",names=["c","d"],sep="\t")

		df_test = df_test.drop(1073).reset_index()
		df_train = df_train.drop(1073).reset_index()
	...
	def remove_noise(self, X_new, kmeans, specialOne):
		self.cleanedId = (np.where(np.array(specialOne) < self.THRESHOLD)[0]).tolist()
	...
	def getKmeansCache(self):
		print("In cache1")
		return self.X_new, self.kmeans, self.specialOne
	...
	def kmeans_cleaning(self, use_cache, cache):
		...
		if self.USE_KMEANS and not use_cache:
			...
			self.X_new = self.kmeans.fit_transform(tdmatrix)   # hope they don't mess with indices of training data.
			corressponding_dists_with_indices_not_messed_hopefully = [self.X_new[(i, x)] for i,x in enumerate(self.kmeans.labels_)]
		...
		# plt.show()	
			...
			self.specialOne = corressponding_dists_with_indices_not_messed_hopefully
			self.remove_noise(self.X_new, self.kmeans, self.specialOne)
			...
			print("finally of some use")
			# self.X_train, self.y_train, self.cleanedId = cache

			# for cl in self.classes:
			# 	self.docsId[cl] = set(self.docsId[cl]).intersection(self.cleanedId) # updating docsId
			X_new, kmeans, specialOne = cache
			self.remove_noise(X_new, kmeans, specialOne)
